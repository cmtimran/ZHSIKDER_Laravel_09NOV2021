...
<div class="container">
  ...
  <p title="Transport Details">{{Session::get('school.system_name')}}( {{Session::get('school.school_eiin')}} ) Faculty Details</p><br/><!-- Transport Details -->
  ...
  <div class="controls text-right">
      ...
      <ul class="nav nav-tabs">
        <li class="active"><a data-toggle="tab" href="#home"><i class="fa fa-bars" aria-hidden="true"></i> Faculty</a></li>
        @permission('INSERT')
        <li><a data-toggle="tab" href="#menu1"><i class="fa fa-plus-circle" aria-hidden="true"></i> Add New Faculty</a></li>
        @endpermission
      </ul>
...
{{--                       <td>{{$department_list->class_name}}</td>--}}
                       ...
                       <td>{{$faculty_list->teacher_name}}</td>
                       <td>{{$faculty_list->description}}</td>
                       <td>{{$faculty_list->created_at}}</td>
                       <td>{{$faculty_list->updated_at}}</td>
                       <td id="my_align" class="display_status">
                        @permission('EDIT')
                        {{Form::open(['url'=>"/manage_faculty/$faculty_list->faculty_id/edit" ,'method'=>'GET'])}}
                          {{Form::submit('Edit',['class'=>'btn btn-primary'])}} 
                        {{Form::close()}}
                      @endpermission

                        @permission('DELETE')
                           {{Form::open(['url'=>"/manage_faculty/$faculty_list->faculty_id" ,'method'=>'DELETE'])}} {{Form::submit('Delete',['class'=>'btn btn-danger','onclick'=>"return confirm('Are you sure you want to Remove ?')"])}} {{Form::close()}}
                        @endpermission
