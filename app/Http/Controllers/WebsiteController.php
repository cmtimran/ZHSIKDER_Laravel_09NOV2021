...
class WebsiteController extends Controller
{
    public function Home()
    {
        ...
        $data['events']   = WebsiteEventsModel::get();
        ...
        $data['news']   = WebsiteNewsModel1::get();
//        dd($data['slider']);
//        dd($data['contact']);
//        dd($data['founder'][0]['name'],$data['chairman']);
...
//        dd($data['website_setup']);
    ...
    {
        $data['teacher'] = teacher_model::join('teacher_address_child','teacher.teacher_id','=','teacher_address_child.parent')->where('designation','principal')->first();
        $data['website_setup'] = WebsiteSetupModel::first();
    ...
    public function student_advisor($id)
    {
//        dd($id);
        $data['student_advisor'] = teacher_model::join('manage_department','teacher.department_id','=','manage_department.id')->where('department_id',$id)->where('job_type','Student Advisor')->first();
        return view('website.faculty_student_advisor', $data);
    }

    public function former_head($id)
    {

        $data['student_advisor'] = former_head_model::join('manage_department','former_head.department_id','=','manage_department.id')->where('former_head.department_id',$id)->first();
    ...
    public function activities($id)
    {
        ...
        $data['student_advisor'] = activities_model::join('manage_department','activities.department_id','=','manage_department.id')->where('activities.department_id',$id)->first();

        return view('website.activities', $data);
    }

    public function schollership($id)
    {

        $data['student_advisor'] = schollership_model::join('manage_department','schollership.department_id','=','manage_department.id')->where('schollership.department_id',$id)->first();

        return view('website.schollership', $data);
    }
...
//        dd($id);
        ...
        $data['department'] = manage_department_model::join('manage_faculty','manage_faculty.faculty_id','=','manage_department.faculty_name')
            ->join('teacher','teacher.teacher_id','=','manage_faculty.chairperson')
        ->where('manage_department.department_code', $id)
            ->first();

        $data['manage_department']  = manage_department_model::get();
        $data['general_settings']   = general_settings_model::first();
        $data['notice']   = notice_board_model::where('notice_department', $id)->get();
        $data['gallery']   = WebsiteEventModel::where('department', $data['department']->id)->get();
//        dd($data['gallery'],$data['department']->id);
        $data['events']   = WebsiteEventsModel::where('department', $data['department']->id)->get();
        $data['departments']   = manage_department_model::all();
        $data['news']   = WebsiteNewsModel1::where('department', $data['department']->id)->get();
...
//        dd($data['news'],$id);
    ...
    public function department_wise_faculty($id)
    {

        $data['teacher_info'] = teacher_model::join('manage_department','teacher.department_id','=','manage_department.id')->where('teacher.department_id', $id)->where('status', 'teacher')->select('teacher.*','manage_department.department_name')->orderBy('sort_index', 'ASC')->get();

        $data['department_name'] = manage_department_model::where('id',$id)->pluck('department_name')->first();
...
//        dd($data['teacher_info']);
    ...
    public function department_wise_tution($id)
    {
        $data['department_name'] = manage_department_model::where('id',$id)->pluck('department_name')->first();

        $data['undergrad_tution_fee'] = Manage_tution_fees_model::where('department_name',$id)->where('program_type','Undergraduate Program')->orderBy('created_at','DESC')->first();

        $data['post_tution_fee'] = Manage_tution_fees_model::where('department_name',$id)->where('program_type','Graduate Program')->orderBy('created_at','DESC')->first();

//        dd($data['undergrad_tution_fee']);
...
//        $requested_data['is_family_member_of_hem_section'] = 'no';
    ...
    {
        $data['gallery'] = WebsiteEventModel::whereIn('type', ['1', '3'])->get();
