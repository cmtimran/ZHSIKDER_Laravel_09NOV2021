...
function getPaths(src, baseDir, output) {
    ...
    if (typeof src == 'string') {
        var srcSegments = parse(src);

        if (!output) {
            // If we have no output at all, then we'll
            // use a src.min.extension convention.
            output = srcSegments.path.replace(srcSegments.ext, '.min' + srcSegments.ext);
        }
    ...
    return new Elixir.GulpPaths().src(src, baseDir).output(output, defaultName);
};
